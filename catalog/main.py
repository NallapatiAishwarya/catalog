...
######
# Showing universities based on byke category
...
@app.route(
    '/University/addUniversityName/addCollegeDetails/<string:ishname>/add',
    methods=['GET', 'POST'])
def addCollegeDetails(ishname):
    ...
    if request.method == 'POST':
        ...
        collegedetails = CollegeName(name=name,
...
def allUniversitiesJSON():
    ...
    for c in range(len(category_dict)):
        universities = [i.serialize for i in session.query(
                        CollegeName).filter_by(
                        universitynameid=category_dict[c]["id"]).all()]
        if universities:
...
def categoryItemsJSON(college_name):
    universityName = session.query(UniversityName).filter_by(
                                   name=college_name).one()
    universities = session.query(CollegeName).filter_by(
                                 universityname=universityName).all()
...
@app.route(
    '/University/<path:university_name>/<path:universitycollege_name>/JSON')
def ItemJSON(university_name, universitycollege_name):
    universityName = session.query(UniversityName).filter_by(
                                   name=university_name).one()
    universityCollegeName = session.query(CollegeName).filter_by(
           name=universitycollege_name, universityname=universityName).one()
